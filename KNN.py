...
def train_knn_model():
    ...
    # Bumping up neighbors slightly stabilizes cross-boundary vote tracking
    ...
    return knn_model, scaler, X_train_scaled.columns, accuracy, f1, roc_auc, cm
...
def predict_knn(model, scaler, expected_columns_order, single_input_df):
    ...
    class_probabilities = model.predict_proba(processed_input)[0]  # Extracts 1D row mapping directly
...
def get_user_input_cli(expected_columns_order):
    ...
    # 📍 FIXED: BMI removed from the features entirely to eliminate CLI test leakage
    user_data = {
        'Gender': gender,
        'Age': age,
        'Height': height,
